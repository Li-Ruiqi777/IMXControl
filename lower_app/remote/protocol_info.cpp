 ...
 * @return NULL
 */
ProtocolInfo::ProtocolInfo(uint8_t *pRxCachebuf, uint8_t *pTxCacheBuf, uint8_t *pRxData, uint16_t nMaxSize)
{
 ...
 * @return 接收数据和校验的结果
 */
int ProtocolInfo::CheckRxBuffer(int nFd)
{
    int nread;
    int CrcRecv, CrcCacl;
 ...
 * @return 执行执行的结果
 */
int ProtocolInfo::ExecuteCommand(int nFd)
{
    uint8_t nCommand;
    ...
    m_TxBufSize = 0;
    ApplicationReg *applicationReg = ApplicationReg::get_instance();

    switch (nCommand)
    {
    case CMD_REG_READ:
        applicationReg->GetMultipleReg(nRegIndex, nRxDataSize, pCacheDataBuf);
        m_TxBufSize = CreateTxBuffer(ACK_OK, nRxDataSize, pCacheDataBuf);
        break;
    case CMD_REG_WRITE:
        memcpy(pCacheDataBuf, &m_RxCacheDataPtr[5], nRxDataSize);
        applicationReg->SetMultipleReg(nRegIndex, nRxDataSize, pCacheDataBuf);
 ...
 * @return 执行执行的结果
 */
int ProtocolInfo::CreateTxBuffer(uint8_t nAck, uint16_t nDataSize, uint8_t *pData)
{
 ...
 * @return 执行执行的结果
 */
int ProtocolInfo::SendTxBuffer(int nFd)
{
 ...
 * @return NULL
 */
uint16_t ProtocolInfo::CrcCalculate(uint8_t *pDataStart, uint16_t nDataSize)
{
    return 0xFFFF;
}
